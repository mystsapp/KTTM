...
namespace KTTM.Controllers
{
    public class TT621sController : BaseController
    {
        ...
        #region KhongTC
        public async Task<IActionResult> KhongTC_141(Guid kvptcId, string strUrl, string maKh, string tenKh, long tamUngId)
        {
            if (tamUngId == 0) // for redirect
            {
                ViewBag.tamUngId = "";
            }
            else
        ...
        public async Task<IActionResult> ThemMoiCT_TT_KhongTC(long tamUngId, long id_Dong_Da_Click,
            string dienGiaiP, string hTTC, string soTienNT) // tamungid == kvctpctid // 1 <-> 1
        {
            ...
            TT621VM.KVCTPTC = await _kVCTPTCService.FindByIdInclude(kVCTPCTId_PhieuC);
            ...
            TT621VM.TT621.SoTienNT = tamUngPhiaTren.ConLaiNT.Value - soTienNT_TrongTT621_TheoTamUng; // kVCTPCT.SoTien trong phieuC
        ...
        public async Task<IActionResult> ThemMoiCT_TT_KhongTC_Post(long tamUngId) // tamungid phia tren khi click
        {
            ...
            //
            decimal soTienNT_CanKetChuyen = _tT621Service.Get_SoTienNT_CanKetChuyen_KhongTC(TT621VM.TT621.TamUngId); // TT621VM.KVCTPCT.SoTienNT tu view qua
        ...
        private async Task ThemMoiCT_TT_KhongTC_Get(long tamUngId)
        {
            TT621VM.TamUngId = tamUngId; //tamungId phia tren khi click
            var tamUng1 = await _tamUngService.GetByIdAsync(tamUngId);
            var kVCTPCTId_PhieuC = tamUng1.Id; // tamung, kvctptc: 1 <--> 1
            TT621 tT621 = _tT621Service.GetDummyTT621_By_KVCTPCT(kVCTPCTId_PhieuC);
            TT621VM.TT621 = tT621;

            TT621VM.TT621.MaKhCo = tamUng1.MaKhNo; // makh cua nguoi tamung
        ...
        public async Task<IActionResult> ThemMoiCT_TT_ContextMenu_KhongTC(long tamUngId, long kVCTPCTId_PhieuTC, long id_Dong_Da_Click) 
        {
            kVCTPCTId_PhieuTC = tamUngId; // kvctptc <-> tamung: 1 <-> 1
        ...
        [HttpPost, ActionName("ThemMoiCT_TT_ContextMenu_KhongTC")]
        public async Task<IActionResult> ThemMoiCT_TT_ContextMenu_KhongTC_Post(long tamUngId) // tamungid phia tren khi click
        {
            if (tamUngId == 0)
                return NotFound();
            var tamUng = await _tamUngService.GetByIdAsync(tamUngId);
            ...
            if (!ModelState.IsValid)
            {
                await ThemMoiCT_TT_ContextMenu_Get(tamUngId);
        ...
        public async Task<JsonResult> GetLoaiTien(long tamUngId)
        {
            KVCTPTC kVCTPTC = await _kVCTPTCService.GetById(tamUngId); // kvctptc <-> tamung : 1 <-> 1

            return Json(kVCTPTC.LoaiTien);
        }
        ...
        public async Task<IActionResult> ThemMoiCT_TT_ContextMenu_Post(long tamUngId) // tamungid phia tren khi click
        {
            ...
            //
            decimal soTienNT_CanKetChuyen = _tT621Service.Get_SoTienNT_CanKetChuyen(
                TT621VM.TT621.TamUngId, TT621VM.KVCTPTC.SoTienNT.Value, kVPTC.MFieu);//.Get_SoTienNT_CanKetChuyen_KhongTC(TT621VM.TT621.TamUngId); // TT621VM.KVCTPCT.SoTienNT tu view qua
            // txtSoTienNT nhập vào không được vượt quá soTienNT_ChuaCapNhat(cũ) + soTienNT_CanKetChuyen (tt621 theo tamung, sotienNT theo phieu TC)
            TT621VM.KVPTC = await _kVPTCService.GetByGuidIdAsync(TT621VM.KVCTPTC.KVPTCId);
            if (TT621VM.TT621.SoTien > soTienNT_CanKetChuyen)
            {
        ...
        private async Task ThemMoiCT_TT_ContextMenu_Get(long tamUngId)
        {
        ...
        [HttpPost]
        public async Task<JsonResult> KetChuyen(long tamUngId, decimal soTienNT_PhieuTC, long kVCTPCTId_PhieuTC, decimal tyGiaTT141) // kVCTPCTId_PhieuTC: tren TT621CreateView
        {
            // from login session
            var user = HttpContext.Session.GetSingle<User>("loginUser");
            ...
            if (soTienCanKetChuyen == 0)
            {
                ...
                {
                    if (kVCTPCTId_PhieuTC == 0) // Khong PTC
                    {
                        decimal? tienTUDau;
                        decimal? tienTUSau;
                        ...
                        tienTUSau = tamUng.SoTienNT * tyGiaTT141; // tyGiaTT141: chi co trong Khong PTC
                    ...
                    else // TT bình thường
                    {
                        ...
                        if (kVCTPCT.KVPTC.MFieu == "C")
                        {
                            ...
                            tienTUDau = tamUng.SoTien + (kVCTPCT.SoTienNT * kVCTPCT.TyGia); // VND
